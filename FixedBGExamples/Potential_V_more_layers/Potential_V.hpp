...
class Potential
{
  public:
    ...
    const double mu_H = 2, lambda = 2, r_0 = 2, k= 10;
    template <class data_t>
	...
	data_t r_0 = r_0;
	//std::cout << simd_compare_gt(r_0,r_BL) << std::endl;
	data_t zero = 0 ;
	data_t m = simd_conditional(simd_compare_lt(r_BL,r_0), zero, sqrt(pow(r_0/(R*pow(1 + r_plus/(4.*R),2)),lambda)
			*(1 - r_0/(R*pow(1 + r_plus/(4.*R),2)))*pow(mu_H,2)));  
	return m;
    }   
	...
	data_t temp = (pow(2,-5 + 4*lambda)/*  *pow(euler,(k*(-r_0 + R*pow(1 + r_plus/(4.*R),2)))/r_0)*/  *
			pow((r_0*R)/pow(r_plus + 4*R,2),-1 + lambda)/* *(k*(r_plus + 4*R)*(-pow(r_plus,2) + 16*(R_squared))
			- 16*(1 + pow(euler,(k*(-r_0 + R*pow(1 + r_plus/(4.*R),2)))/r_0))*r_0*(4*pow(x,2) + 4*(pow(y,2) + pow(z,2)) 
		        - r_plus*R)*lambda)*/ *pow(mu_H,2))/1.0 /* (pow(1 + pow(euler,(k*(-r_0 + R*pow(1 + r_plus/(4.*R),2)))/r_0),2)*(R_squared)
		   *pow(r_plus + 4*R,3)  *sqrt((pow(16,lambda)*pow((r_0*R)/pow(r_plus + 4*R,2),lambda)*pow(mu_H,2))/
			  (1 + pow(euler,-(k*(-r_0 + R*pow(1 + r_plus/(4.*R),2)))/r_0))) )*/;
	temp = exp( (k*(-r_0  /* + R * pow(1 + r_plus/(4.*R),2) */ ))/r_0);
	//now the monstrosity z derivative:
	
	if (  simd_compare_gt(r_0,r_BL)  ){return 0;}
   ...
   {   
	...
	//need to write function that corresponds to the m_field above
	data_t temp = -((-r_plus + 4*R)*((pow(r_plus,2) + 16*(R_squared))*lambda 
			+ 8*R*(r_plus*lambda - 2*r_0*(1 + lambda))))/(2.*(R_squared)
		*(r_plus + 4*R)*(pow(r_plus,2) + 8*r_plus*R + 16*(R_squared - r_0*R)));


	data_t zero = 0;	
	partial_log_m[0] = simd_conditional(simd_compare_lt(r_BL,r_0),zero, temp*x )  ;
	partial_log_m[1] = simd_conditional(simd_compare_lt(r_BL,r_0),zero, temp*y )  ;
	partial_log_m[2] = simd_conditional(simd_compare_lt(r_BL,r_0),zero, temp*z )  ;
