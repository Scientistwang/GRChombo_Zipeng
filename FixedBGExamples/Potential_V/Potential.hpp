...
class Potential
{
  public:
	...
	data_t m = sqrt((pow(r_0/(R*pow(1 + r_plus/(4.*R),2)),lambda)*pow(mu_H,2))/
   ...
   {   
	...
	//need to write function that corresponds to the m_field above
	data_t temp = (((k*(-pow(r_plus,2) + 16*(R_squared)))*
			pow(euler,(k*(r_0 - R*pow(1 + r_plus/(4.*R),2)))/r_0)/
			((1 + pow(euler,(k*(r_0 - R*pow(1 + r_plus/(4.*R),2)))/r_0))*r_0) 
			- (16*(4*pow(x,2) + 
			4*(pow(y,2) + pow(z,2)) - r_plus*R)*lambda)/(r_plus + 
			4*R)))/(32.*pow(R_squared,1.5));
