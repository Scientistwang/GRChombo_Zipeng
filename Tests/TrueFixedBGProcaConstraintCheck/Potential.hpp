...
class Potential
{
  public:
    ...
    const double mu_H = 2, lambda = 2, r_0 = 8, k= 20, mu_c = 0.1;
    template <class data_t>
	...
	data_t m = sqrt(pow(mu_c,2) + (pow(r_0/(R*pow(1 + r_plus/(4.*R),2)),lambda)*pow(mu_H,2))/(1 + pow			(euler,-(k*(-r_0 + R*pow(1 + r_plus/(4.*R),2)))/r_0)));
	return m;
    }   
   ...
   {   
	...
	//need to write function that corresponds to the m_field above
	data_t temp = (pow(2,-5 + 4*lambda)*pow((r_0*R)/pow(r_plus + 4*R,2),-1 + lambda)
			*(pow(euler,(k*(r_0 - R*pow(1 + r_plus/(4.*R),2)))/r_0)*k*(r_plus + 4*R)*
			(-pow(r_plus,2) + 16*(R_squared)) - 16*(1 + pow(euler,(k*(r_0 - 
			R*pow(1 + r_plus/(4.*R),2)))/r_0))*r_0*(4*pow(x,2) + 4*pow(y,2) + 4*pow(z,2) 
			- r_plus*R)*lambda)*pow(mu_H,2))/((1 + pow(euler,(k*(r_0 - R*pow(1 + r_plus/
			(4.*R),2)))/r_0))*(R_squared)*pow(r_plus + 4*R,3)*((1 + pow(euler,(k*(r_0 - 
			R*pow(1 + r_plus/(4.*R),2)))/r_0))*pow(mu_c,2) + pow(16,lambda)*
			pow((r_0*R)/pow(r_plus + 4*R,2),lambda)*pow(mu_H,2)));
