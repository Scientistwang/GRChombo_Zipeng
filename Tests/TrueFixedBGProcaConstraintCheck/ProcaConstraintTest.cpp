...
int main()
{
	...
	IsotropicKerrFixedBG::params_t kerr_params;
	    // metric background
	    kerr_params.mass = 0.5;
	    kerr_params.spin = 0.25;
	    kerr_params.center = center_vector;

	    IsotropicKerrFixedBG kerr_metric(kerr_params, dx);
	    ...
	    Potential potential(potential_params, kerr_params);
        ...
        BoxLoops::loop(
            InitialConditions<IsotropicKerrFixedBG>(
        ...
        FixedBGProcaConstraintTest<Potential, IsotropicKerrFixedBG> my_constraint(
            kerr_metric, dx, proca_mass, proca_damping, potential);
        ExcisionProcaTest<ProcaField, IsotropicKerrFixedBG> excision(
