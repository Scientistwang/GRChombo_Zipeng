...
class Potential
{
  public:
	...
	data_t euler = M_E;
        ...
        data_t m = sqrt(pow(mu_c,2) + ((pow(x,2) + pow(y,2))*pow(r_plus/(R*pow(1 + r_plus/(4.*R),2)),lambda)*pow(mu_H,2))/(R_squared));	
	...
	data_t temp_x=	-((pow(2,-1 + 4*lambda)*x*pow((r_plus*R)/pow(r_plus + 4*R,2),lambda)*(4*R*(-2*pow(z,2) + (pow(x,2) + pow(y,2))*lambda) - r_plus*(2*pow(z,2) + (pow(x,2) + pow(y,2))*lambda))*pow(mu_H,2))/(pow(R_squared,2)*(r_plus + 4*R)*sqrt(pow(mu_c,2) + (pow(16,lambda)*(pow(x,2) + pow(y,2))*pow((r_plus*R)/pow(r_plus + 4*R,2),lambda)*pow(mu_H,2))/(R_squared))));
	data_t temp_y=	-((pow(2,-1 + 4*lambda)*y*pow((r_plus*R)/pow(r_plus + 4*R,2),lambda)*(4*R*(-2*pow(z,2) + (pow(x,2) + pow(y,2))*lambda) - r_plus*(2*pow(z,2) + (pow(x,2) + pow(y,2))*lambda))*pow(mu_H,2))/(pow(R_squared,2)*(r_plus + 4*R)*sqrt(pow(mu_c,2) + (pow(16,lambda)*(pow(x,2) + pow(y,2))*pow((r_plus*R)/pow(r_plus + 4*R,2),lambda)*pow(mu_H,2))/(R_squared))));
	data_t temp_z = -((pow(2,-1 + 4*lambda)*(pow(x,2) + pow(y,2))*z*pow((r_plus*R)/pow(r_plus + 4*R,2),lambda)*(-(r_plus*(-2 + lambda)) + 4*R*(2 + lambda))*pow(mu_H,2))/(pow(R_squared,2)*(r_plus + 4*R)*sqrt(pow(mu_c,2) + (pow(16,lambda)*(pow(x,2) + pow(y,2))*pow((r_plus*R)/pow(r_plus + 4*R,2),lambda)*pow(mu_H,2))/(R_squared))));
	//need to write function that corresponds to the m_field above
    	partial_m[0] = temp_x;
    	partial_m[1] = temp_y;
    	partial_m[2] = temp_z;
